...
export default {
  ...
  methods: {
    async updatePost () {
      ...
      try {
        await this.$api.put(`${API_ROUTES.POST_UPDATE}`, { id: this.$route.params.id, title: this.title, content: this.content, medias: this.medias })
  ...
  async mounted () {
    try {
      const { data } = await this.$api.get(`${API_ROUTES.POST_POST_ID}/${this.$route.params.id}`)
